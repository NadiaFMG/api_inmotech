...
import VistaMapa from '../components/map/VistaMapa';
import '../styles/VistaMapa.css';
// Cambia axios por el servicio de búsqueda
import { busquedaInmuebleService } from '../services/propertyService';
...
function Inmuebles() {
    ...
    useEffect(() => {
        const fetchInmuebles = async () => {
            ...
            setError(null);
            try {
                // Unifica todos los filtros y paginación
                const params = {
                    page: paginaActual,
                    limit: inmueblesPorPagina,
                    ...(searchParams || {}),
                ...
                // Usa el servicio conectado a /fc5/busqueda
                const response = await busquedaInmuebleService.buscar(params);
                // Ajusta según la estructura de tu backend
                setInmuebles(response.data.inmuebles || response.data || []);
                setTotalPaginas(
                    response.data.total
                        ? Math.ceil(response.data.total / inmueblesPorPagina)
                        : 1
                );
            } catch (err) {
                setError('Error al cargar los inmuebles');
